...
use app\portal\model\PortalTeacherModel;
...
//        }
        ...
        $data        = $postService->adminTeacheList($param);
...
//            $post['email'] = '123123';
//            $post['more'] = '123123';
//            $post['create_time'] = '123123';
//            $post['website'] = '123123';
//            $post['content'] = '123123';
            $post = $data['post'];
            $result = $this->validate($post, 'AdminTeacher');
...
//            $portalPostModel = new PortalTeacherModel();
...
//            }
            ...
            $portalPostTeachModel = new PortalTeacherModel();

            $id = $portalPostTeachModel->adminAddTeacher($data['post']);

            if($id){
                $this->success('添加成功!', url('AdminTeacher/edit', ['id' => $id]));
            }else{
                $this->error('添加失败!', url('AdminTeacher/add'));
            }
...
//        $portalPostModel = new PortalPostModel();
        ...
        $post            = $portalPostTeachModel->where('id', $id)->find();
...
//        $postCategoryIds = implode(',', array_keys($postCategories));
        ...
        if($post['teacher_sex']==1){
            $this->assign('checked1','checked');
            $this->assign('checked2','');
            $this->assign('checked3','');
        }else if($post['teacher_sex']==2){
...
//            unset($data['post']['recommended']);
            ...
            $portalPostTeachModel->adminEditTeacher($data['post']);
...
//            $result       = $portalPostModel->where('id', $id)->find();
            $result = Db::name('portalTeacher')->where('id', $id)->find();

            $data         = [
                'object_id'   => $result['id'],
                'create_time' => time(),
                'table_name'  => 'portal_teacher',
...
//                ->update(['delete_time' => time()]);
            $resultPortal = Db::name('portalTeacher')->where('id', $id)->update(['delete_time' => time()]);
